  ...
  run(code) {
    ...
    let getPrec = (op) => {
      for (let i = 0; i < this.precedence.length; i++) {
        if (this.precedence[i].operator == op) return this.precedence[i].precedence;
      }
      return null;
    };
    ...
    let getAssociativity = (op) => {
      for (let i = 0; i < this.precedence.length; i++) {
        if (this.precedence[i].operator == op) return this.precedence[i].associativity;
      }
      return null;
    };
        ...
        while (j < this.lines[i].line.length) {
          if (this.lines[i].line[j].type == "integer" || this.lines[i].line[j].type == "float") {
            ...
            while (
              ...
              getPrec(operators[operators.length - 1]) >
                getPrec(
                  this.lines[i].line[j].value ||
                    (getPrec(operators[operators.length - 1]) == getPrec(this.lines[i].line[j].value) &&
                      getAssociativity(this.lines[i].line[j].value) == "left")
                )
            ) {
              output.push(operators.pop());
            }
